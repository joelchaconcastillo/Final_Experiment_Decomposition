...
my $Df = 0.5;
my $pops=100;
my $max_nfes=25000*$pops;
my @CR_G = ("0.0", "0.25", "0.5", "0.75", "1.0");
my @F_G = ("0.25", "0.5", "0.75", "1.0");
   ...
   @Instance = ("WFG1", "WFG2", "WFG3", "WFG4", "WFG5", "WFG6", "WFG7", "WFG8", "WFG9");
      foreach(@Instance)
      {
      	...
      	for(my $nobj = 2; $nobj <=3; $nobj++)
      	{
      	   ...
      	   for(my $Sed = 1; $Sed <=35; $Sed++) ##Realizar 35 ejecuciones con distintas semilla de cada instancia..
      	   {
      	   	print $fout "~$PathAlgorithm/Ejecutable $PathAlgorithm $_ $Sed $nobj $pops $max_nfes $CR $F $l $k $Di $Df\n";
      	   }
      	}
      }
