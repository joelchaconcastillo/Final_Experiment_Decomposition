...
//
		...
		double rate2 =0.5;// box_muller(0.5,0.1);
...
void CMOEAD::exec_emo(int run)
{
	...
	sprintf(filename1,"%s/POS/v2_POS_MOEAD_%s_RUN%d_seed_%d_nobj_%d_nvar_%d_DI_%lf_DF_%lf",strpath, strTestInstance,run, seed, nobj, nvar, Di/sqrt(nvar), Df);
	sprintf(filename2,"%s/POF/v2_POF_MOEAD_%s_RUN%d_seed_%d_nobj_%d_nvar_%d_DI_%lf_DF_%lf",strpath, strTestInstance,run, seed, nobj, nvar, Di/sqrt(nvar), Df);
