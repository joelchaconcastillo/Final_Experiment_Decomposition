...
void CMOEAD::mate_selection(vector<int> &list, int cid, int size, int type){
    ...
    while(list.size()<size)
	{
		if(type==1){
			...
			parent  = rand()%(int) population.size();//int(population.size()*rnd_uni(&rnd_uni_init));
...
//		diff_evo_xoverB(population[c_sub].indiv,population[plist[0]].indiv,population[plist[1]].indiv, child, rate2);
		diff_evo_xoverA(population[c_sub].indiv,population[plist[0]].indiv,population[plist[1]].indiv, population[plist[2]].indiv, child, rate2);
		plist.clear();
